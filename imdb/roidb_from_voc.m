function roidb = roidb_from_voc(imdb,method)
...
% ---------------------------------------------------------
%L
cache_file = ['./imdb/cache/roidb_' imdb.name '_' method];
try
  load(cache_file);
catch
...
%  roidb.name = strcat(imdb.name,'_',method);

  fprintf('Loading region proposals...');
%L  
  regions_file = sprintf('./data/%s/%s', method,roidb.name);
