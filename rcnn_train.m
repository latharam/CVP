function [rcnn_model, rcnn_k_fold_model] = ...
    rcnn_train(method,imdb, varargin)
...
opts.feat_norm_mean = rcnn_feature_stats(imdb, opts.layer, rcnn_model,method);
...
save_file = sprintf('./feat_cache/%s/%s/%s/gt_pos_layer_5_cache.mat', ...
    method,rcnn_model.cache_name, imdb.name);
try
  load(save_file);
  fprintf('Loaded saved positives from ground truth boxes\n');
catch
  [X_pos, keys_pos] = get_positive_pool5_features(imdb, opts,method);
...
for hard_epoch = 1:max_hard_epochs
  for i = 1:length(imdb.image_ids)
    ...
    % more than once)
    [X, keys] = sample_negative_features(method,first_time, rcnn_model, caches, ...
        imdb, i);
...
% ------------------------------------------------------------------------
function [X_neg, keys] = sample_negative_features(method,first_time, rcnn_model, ...
                                                  caches, imdb, ind)
% ------------------------------------------------------------------------
opts = rcnn_model.training_opts;

d = rcnn_load_cached_pool5_features(opts.cache_name, ...
    imdb.name, imdb.image_ids{ind}, method);
...
% ------------------------------------------------------------------------
function [X_pos, keys] = get_positive_pool5_features(imdb, opts, method)
% ------------------------------------------------------------------------
...
for i = 1:length(imdb.image_ids)
  ...
  d = rcnn_load_cached_pool5_features(opts.cache_name, ...
      imdb.name, imdb.image_ids{i},method);
